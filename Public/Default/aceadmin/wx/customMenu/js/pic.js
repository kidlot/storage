...
function showList(items){
	...
	leftPicObj.html('');//左右布局
	rightPicObj.html('');
	var leftHeight = 0;
	var rightHeight = 0;
	for(var i in items){
		var item = items[i];
		var thumb = 'http://yeeyi.com/bbs/data/attachment/forum/mobile/'+item.thumb;//图片路径
		//可以使用图片缓存
		...
		if(leftHeight > rightHeight){
			//如果右侧高度小，则追加到右侧
			var trHead = '<div class="blockRight">'+'<div class="mask_page">'+'</div>'+'<i class="icon_card_selected">已选择</i>';
			var trPic = '<img style="min-height:'+item.picHeight+'em" src="'+thumb+'" id="p'+item.tid+'" class="list_img">';
			var trTitle = '<div class="pictitle"><div class="subject">'+"1"+'</div>';
			...
			//反之，如果右侧高度大，则追加到左侧
			var trHead = '<div class="blockLeft">'+'<div class="mask_page">'+'</div>'+'<i class="icon_card_selected">已选择</i>';
			var trPic = '<img style="min-height:'+item.picHeight+'em" src="'+thumb+'" id="p'+item.tid+'" class="list_img">';
			var trTitle = '<div class="pictitle"><div class="subject">'+"1"+'</div>';
			var trAddinfo = '<div class="addinfo"><div class="author">'+b64.decode(item.author)+'</div><div class="view">View('+item.views+')</div> </div></div></div>';
			tr = trHead + trPic + trTitle + trAddinfo;
...
function showMoreList(items){
	...
	for(var i in items){
		...
		if(leftHeight > rightHeight){
			//var mask='<div class="mask_page">'+'</div>';
			var trHead = '<div class="blockRight">'+'<div class="mask_page">'+'</div>'+'<i class="icon_card_selected">已选择</i>';
			var trPic = '<img style="min-height:'+item.picHeight+'em" src="'+thumb+'" id="p'+item.tid+'" class="list_img">';
			var trTitle = '<div class="pictitle"><div class="subject">'+"1"+'</div>';
			var trAddinfo = '<div class="addinfo"><div class="author">'+b64.decode(item.author)+'</div><div class="view">View('+item.views+')</div> </div></div></div>';
